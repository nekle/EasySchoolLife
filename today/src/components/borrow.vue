<template>
  <el-container style="height: 700px; border: 1px solid #eee">
    <el-container>
          <el-header style="text-align: center; font-size: 24px">我的租入</el-header>
          <el-main>
                      ...
                      <div class="demo-image">
                        ...
                        <el-container direction="vertical">
                          <el-image
                            ...
                            :fits="fill"
                            @click="accountInfo(account.information.account,account.information.password,account.money,account.userid,account.create_time,account.information.type)"
                          ></el-image>
                          <el-container>
                            <span>{{account.information.type}}</span>
                          </el-container>
                          <el-container direction="vertical">
                            <el-container>
...
export default {
  ...
  mounted () {
    axios.post('http://' + this.$ip + ':' + this.$port + '/user/account/getinfo').then(res => {
      ...
      this.accounts = jsonObj.data.borrow
      // 将数据的长度赋值给totalCount
      this.pagesum = Math.ceil(jsonObj.data.borrow.length / this.eachpage)
